...
void main() {
	...
	if ( isMouseDown != 0 &&
		...
		if (brushType == FUSE) {
			...
			cell = ivec4( FIRE, 0, int((random(coord)+1.0) * 64.0 ), 255);
		...
		if (fire > 0) {
			cell = ivec4(FIRE, fire, int(random(coord)+1.5) * FIRE_LIFE, 255);
		}
		...
		if ( neighbor( 0.0, -dy ).r >= FIRE ||
			...
			cell = ivec4( FIRE, 0, int( (random(coord)+1.0) * 128.0 ), 255 );
		}
